...
Shriram Krishnamurthi and Mitchell Wand)~\cite{pombrio-scope}.\marginpar{%
Our notation and terminology in this chapter was influenced by work
published by Erdweg et al.~\cite{erdweg-hygiene} and Neron et al.~\cite{neron-scope}.
}
...
\Cref{sec:rscope-rules} describes the full version, and
...
and shadowing because the definitions are analogous.}
  ...
  \DeclXj \in \min \St{\DeclXk}{\DeclXi \< \DeclXk} \text{ and } \DeclXj \not\< \DeclXi
  \]
\end{definition}
...
\begin{proof}
  ...
  \DeclXj \< \DeclXi$, and $\DeclXj \not\< \DeclXi$, so $\DeclXi$ cannot be in this set, and $\RefnXk$
...
of the subterms of $e$. With this in mind, we offer four kinds of bindings:\marginpar{%
See \cref{sec:rscope-related-scope} for a comparison of our binding
language to attribute grammars.
}
\begin{enumerate}
\item[A.] \SpecBind{j}{i}: A term may make all declarations
...
\end{tikzScopeDiagram}
\end{center}
In a potentially confusing change, these port arrows are drawn in the
opposite direction as the port arrows in the paper this chapter is
based on~\cite{pombrio-scope}. Why the change?

In the paper, imports were drawn as $\ImSymbBackwards$ and exports as
$\ExSymbBackwards$. As a result, in the diagram the up arrow B
connected two downward-pointing import arrows, and the down arrow C
connected two upward-pointing export arrows. Eelco Visser pointed out
how confusing this was, so we switched it.

Relatedly, why do our (solid/green) binding arrows go in the direction
they do? Some considerations suggest that they should go from
reference to declaration (which is what we settled on):
\begin{itemize}[noitemsep,topsep=0em]
  \item They should go the intuitive way, and flow from the natural
    question to ask (where is this reference bound?) to its answer
    (here is the binding declaration).
  \item They should be consistent with related work, and go in the
    same direction as the arrows in Scope Graphs~\cite{neron-scope}.
\end{itemize}
Other considerations suggest that they go in the opposite direction
(from declaration to reference):
\begin{itemize}[noitemsep,topsep=0em]
  \item They should go the intuitive way, and flow in the same
    direction as values do: first a value is bound to a declaration,
    and then later that value is propogated to its reference.
  \item They should be consistent with related work, and match the
    arrows that the DrRacket editor~\cite{drscheme} draws between
    references and declarations.
\end{itemize}
It was a difficult choice, and could reasonably have gone in the other direction.
...
\]
  ...
  \[
    \hspace{5em}
    \inference[S-Shadow]
        {\DeclX[j] \in \min \St{\DeclX[k]}{\SaysScope{e}{\DeclX[i]}{\DeclX[k]}}
          \text{ and } \SaysScopeNot{e}{\DeclX[j]}{\DeclX[i]}}
...
%  &&where $(x, y)$ means a constraint ``$x$ iff $y$''
  \vspace{0.9em} \\
  $\Op{genConj}{a \< b,\; p}$
...
\subsection{Ensuring Hygiene}\label{sec:rscope-hygiene-overview}
...
Neron et al. describe \emph{scope graphs}, which are based on a
similar view, but are more general. For example, they include mechanisms for handling
module scope, which gives them the ability to model both modules and
also other constructs like objects and field lookup.
While our {\sap} binding language can be extended to
handle modules, doing so breaks the transitivity
assumption that we need to infer scope, so we have left it out of
this chapter and consider this a problem for
future work.

\paragraph{Binding Specification Languages}
\label{sec:rscope-related-binding}
...
There is a close analogy between our ports and attributes in attribute
grammars~\cite{knuth-attribute-grammar}: namely, imports are analogous
to inherited attributes and exports are analogous to synthesized
attributes.  The paths between imports and exports that are allowed by
our binding language (e.g., child export to parent export, but not
child export to parent import) are precisely the relationships between
inherited and synthesized attributes that are allowed in attribute
grammars. Most algorithms for evaluating attribute grammars disallow
cycles, however, while our preorders allow them.
...
There are a number of weaknesses in our binding language that it would
...
\begin{itemize}[noitemsep]
  ...
  \item References are not always bound to a single declaration.  For
    ...
    binding determined at runtime. In \cref{def:rscope-ambig}, we
    define a reference that is bound to multiple declarations to be
    \emph{ambiguously bound}, and we consider this to be an error.
    It would be useful to be able to relax this restriction in certain
    circumstances, and infer during scope inference when it should be
    relaxed for a particular sugar.
  \item The ability to group several bindings together into a single
    namespace---\`a la \emph{module declarations} in Scope
    Graphs~\cite{neron-scope}---is necessary for inferring scope for
    modules and for classes. This change is a relatively
    straightforward extensions to our binding language, but is a
    research question when applied to scope inference.
\end{itemize}
