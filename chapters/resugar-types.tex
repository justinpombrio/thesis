...
TODO
\begin{itemize}
\item Discussion and conclusion, second paragraph.
\end{itemize}
...
In this chapter, we focus on the interaction between desugaring and
\emph{type checking}. Type checking occurs either before or after
...
To distinguish this from ordinary type
inference, in which \emph{types} are inferred \emph{within a program},
we will call our inference process \textbf{type resugaring}\footnote{
  We take the term ``resugaring'' from Pombrio and Krishnamurthi~\cite{pombrio-resugar}.
}, in which
...
complex example, an \Code{or} sugar\footnote{
...
\begin{prooftree}
  \RZero{}
  \dashedLine\ROne[t-premise]{\squeeze{0.1em}{\saysTtt{\Gamma}{$\alpha$}{A}}}
  \RZero{}
  \ROne[t-id]{\saysTtt{\Gamma,x:A}{x}{A}}
  ...
  \ROne[t-id]{\saysTtt{\Gamma,x:A}{x}{A}}
  \RZero{}
  \dashedLine\ROne[t-premise]{\squeeze{0.1em}{\saysTtt{\Gamma,x:A}{$\beta$}{B}}}
...
\emph{All of the examples in this chapter run in our implementation},
...
\begin{proof}
  ...
  \textit{Base case: $e$ is an atomic expression $k$}:\\
  $\Desugar{\Subs{\gamma}{k}} = k = \Subs{\Desugar{\gamma}}{k}$.
...
by modifying \Code{t-premise} to only work on judgments so limited:\footnote{
...
desugared code (with the given type).\footnote{
...
as well as a new \Code{t-sub-premise} rule.\footnote{
...
rules (transcribed into Haskell syntax)\footnote{``hlc'' stands for ``Haskell
list comprehension''.}:
...
Similar work has been done for \emph{scope rules}. Herman and Wand
present $\lambda_m$, in which scope annotations on macros are
statically checked~\cite{herman-hygiene}. $\lambda_m$ ensures that if a surface term
is well-scoped according to the annotations, then after desugaring it
will still be well-scoped. Stansifer and Wand continue in this
direction with a more powerful system called Romeo~\cite{stansifer-romeo}. Pombrio and
Krishnamurthi~\cite{pombrio-scope} go further, \emph{inferring} rather than checking
surface scope annotations, directly analogous to our work.
...
and the underlying type system in \cref{sec:rtype-req-type,sec:req-resugar}. We also presented
...
This appendix shows SweetT in action. \cref{fig:rtype-demo} shows a sample
usage of SweetT, and \cref{fig:rtype-sample1,fig:sample2,fig:sample3} show
...
with those in the body of the paper, make use of all of the TAPL type
