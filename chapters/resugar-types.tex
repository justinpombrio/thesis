...
Shriram Krishnamurthi)~\cite{pombrio-resugar-types}.
...
Wyk et al. introduce \emph{forwarding} for attribute
grammars~\cite{attribute-grammar-forwarding}.  Forwarding is a kind of
inheritance. With it, instead of specifying all of its attributes
directly, a node in the \Sc{ast} can instead opt to \emph{forward}
some of them to \emph{a new node of its own construction}. If the
original node is thought of as a surface node, and the new node is
thought of as a core node, then this is a kind of desugaring, and
(some of) the attributes of the core node are automatically propogated
to the surface node. Thus, if these forwarded attributes include scope
or type information, then the surface node's scope or type has been
defined to be the scope or type of its ``desugaring''. Notice that
this differs from resugaring, though: we lift scope \emph{rules} (in
the previous chapter), and type \emph{rules} (in this chapter).
