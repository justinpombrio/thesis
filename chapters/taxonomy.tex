...
\begin{description}
  ...
  \item[Phase] \emph{When} does desugaring occur? Is it at compilation time,
    or at evaluation time (as in staged metaprogramming
    systems)?\marginpar{%
    As previously noted, if the ``phase'' at which desugaring happens
    is evaluation time, then the system isn't really a desugaring system,
...
at runtime via calls to \Code{run}. For all of these reasons,
...
[CHECK,CITE], started the research into macro hygiene [TODO: lisps in
  general, not Scheme specifically, rephrase][CITE], and set
...
guide discusses hygiene, but in testing the macro system is unhygienic
outside of the specific situations described.) Beyond this, Julia also
allows arbitrary runtime metaprogramming: constructing,
deconstructing, and evaluating code at runtime.
