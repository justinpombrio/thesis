...
\item ``desugaring'' vs. ``metaprogramming''
\item mention macro-defining-macros in expressiveness?
\end{itemize}

There are a bewildering variety of desugaring systems.
In this chapter, we categorize them into a taxonomy.
...
\begin{description}
  ...
  \item[Expressiveness] How expressive is it? Can a sugar take an
    ...
    (\Sc{Argument})? If it can take an expression, can it
...
\item[\Sc{oi}] desugaring is superficially similar to lazy
  ...
  arguments (which are pieces of syntax), it does not force that
  argument to be desugared! Rather, the case analysis happens on the
  uninterpreted syntax of that argument.
...
\item[\Sc{io}] desugaring is similar to eager evaluation: the
  ...
  (ii) sugars cannot deconstruct their arguments, so the desugaring
  ...
  their arguments: for example, C++ templates can be used to derive
...
system:
\begin{description}
\item[Arguments] What kind of arguments can be passed to a sugar? In
  ...
  values (e.g., numbers) as arguments.
\item[Result] What kind of syntax can a sugar desugar to? Can it be
  ...
  only?
\item[Deconstruction] Sugars are given syntax as arguments. Can they
  ...
  arguments and desugaring order (OI vs. IO). With OI order, the
  arguments that are deconstructed are in the surface language, since
  they haven't been desugared yet. Under IO order, however, the
  arguments are in the core language, because they \emph{have} been
  desugared.
\end{description}
